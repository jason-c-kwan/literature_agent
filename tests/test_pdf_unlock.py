...
async def test_resolve_content_unlocks_nejm_pdf_with_param(
    ...
    # Add mock for /epdf/ variant which might be generated and tried - ensure it matches the lowercase path
    nejm_epdf_url_generated = "https://www.nejm.org/doi/epdf/10.1056/nejmoa2501440" # as generated by resolve_pdf_link.py
    respx_mock.get(nejm_epdf_url_generated).respond(status_code=403)
    # Also mock the /pdfdirect/ variant if it could be generated
    nejm_pdfdirect_url_generated = "https://www.nejm.org/doi/pdfdirect/10.1056/nejmoa2501440"
    respx_mock.get(nejm_pdfdirect_url_generated).respond(status_code=403)

    # Mock variants with default param from config
    default_param = mock_pdf_unlock_config.return_value['default'][0] # 'param=default_val'
    respx_mock.get(f"{nejm_base_url}?{default_param}").respond(status_code=403)
    respx_mock.get(f"{nejm_epdf_url_generated}?{default_param}").respond(status_code=403)
    respx_mock.get(f"{nejm_pdfdirect_url_generated}?{default_param}").respond(status_code=403)
    # Mock HEAD for these as well, in case HEAD is tried first for them
...
async def test_acs_epdf_unlock(
    ...
    # Mocking behavior:
    # 1. HEAD for /pdf/ path might return 403 or non-pdf
    respx_mock.head(acs_pdf_url).respond(status_code=403)
    # 2. HEAD for /epdf/ path indicates PDF
    respx_mock.head(acs_epdf_url).respond(status_code=200, headers={"Content-Type": "application/pdf"})
    # 3. HEAD for /pdfdirect/ might 404
    respx_mock.head(f"https://pubs.acs.org/doi/pdfdirect/{acs_base_doi_path}").respond(status_code=404)
...
async def test_wiley_needaccess_unlock(
    ...
    respx_mock.head(wiley_unlocked_url).respond(status_code=200, headers={"Content-Type": "application/pdf"})

    # GET requests:
    # 1. Base URL (no params) returns 403
    respx_mock.get(wiley_base_url).respond(status_code=403)
    # Add mock for /epdf/ and /pdfdirect/ variants which might be generated and tried
    wiley_epdf_url_generated = wiley_base_url.lower().replace("/pdf/", "/epdf/")
    respx_mock.get(wiley_epdf_url_generated).respond(status_code=403)
    wiley_pdfdirect_url_generated = wiley_base_url.lower().replace("/pdf/", "/pdfdirect/")
    respx_mock.get(wiley_pdfdirect_url_generated).respond(status_code=403)
    
    # Also mock epdf and pdfdirect variants *with* the domain-specific unlock param (?needAccess=true), ensure they fail if tried out of order
    wiley_unlocked_epdf_url = wiley_unlocked_url.lower().replace("/pdf/", "/epdf/") # This is base_url + ?needAccess=true, then /pdf/ -> /epdf/
    respx_mock.get(wiley_unlocked_epdf_url).respond(status_code=403) 
    wiley_unlocked_pdfdirect_url = wiley_unlocked_url.lower().replace("/pdf/", "/pdfdirect/") # This is base_url + ?needAccess=true, then /pdf/ -> /pdfdirect/
    respx_mock.get(wiley_unlocked_pdfdirect_url).respond(status_code=403)

    # Mock variants with default param from config
    default_param = mock_pdf_unlock_config.return_value['default'][0] # 'param=default_val'
    respx_mock.get(f"{wiley_base_url}?{default_param}").respond(status_code=403)
    respx_mock.get(f"{wiley_epdf_url_generated}?{default_param}").respond(status_code=403)
    respx_mock.get(f"{wiley_pdfdirect_url_generated}?{default_param}").respond(status_code=403)
    # Mock HEAD for these as well
    ...
    if 'onlinelibrary.wiley.com' not in mock_pdf_unlock_config.return_value:
        mock_pdf_unlock_config.return_value['onlinelibrary.wiley.com'] = []
    
    # Add 'needAccess=true' if not already present for the domain
    # This ensures it's specifically a domain rule, not just a default one.
    if 'needAccess=true' not in mock_pdf_unlock_config.return_value['onlinelibrary.wiley.com']:
        mock_pdf_unlock_config.return_value['onlinelibrary.wiley.com'].append('needAccess=true')
